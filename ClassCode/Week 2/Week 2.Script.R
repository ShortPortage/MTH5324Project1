...
sum(msleep$sleep_total <= 7.85)
sum(msleep$sleep_total <= 7.85)/length(msleep$sleep_total)

cor(msleep$sleep_total, msleep$sleep_rem)
cor(msleep$sleep_total, msleep$sleep_rem, use = "complete.obs")

sum(msleep$sleep_rem)
sum(msleep$sleep_rem, na.rm = TRUE)
sleep_rem.clean = msleep$sleep_rem[!is.na(msleep$sleep_rem)]
sum(sleep_rem.clean)
...
boxplot(sleep_total ~ factor(vore,exclude = NULL), data = msleep)
